...
function SendCode() {
  ...
  return <>
    <Box sx={{ width: "100%", height: "100vh", backgroundColor: "rgba(128,128,128,0.2)", display:"flex", justifyContent:"center", alignItems:"Center" }}>
      <Box sx={{ overflowY: "auto", display: "flex", flexDirection: "column", gap: 5, width: isXs ? "100%" : isSm ? "80%" : isMd ? "60%" : "40%", border: "3px solid #66bb6a", backgroundColor: mode==='dark'?'dark':'#fff', boxShadow: "0px 0px 30px gray", height: downSm ? "100vh" : "80vh" }}>
        <Box sx={{ padding: "15px 0", backgroundColor: "#66bb6a", borderBottomRightRadius: "30%", borderBottomLeftRadius: "30%", display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", gap: "5px" }}>
        ...
        <Box sx={{ textAlign: "center", display: "flex", flexDirection: "column", alignItems: "center", gap: 5, marginBottom:"50px" }}>
          ...
          <Box component={"form"} onSubmit={handleSubmit(submit)} sx={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 2, width: "80%" }}>
              <TextField label={t('Email')} type='email' fullWidth placeholder={`${t('Enter')}${' '}${t('Email')}${' '}${t('Please')}${' !'}`} color='success' variant="outlined" {...register("email")} error={errors.email} helperText={t(errors.email?.message)} />
              <Button type='submit' variant="contained" fullWidth sx={{ fontSize: '16px', backgroundColor:"#66bb6a", color:"#fff" }} disabled={isSubmitting}>{isSubmitting ? <CircularProgress sx={{color:"#80b501"}} /> : t('Send Code')}</Button>
          </Box>
        </Box>
      </Box>
    </Box>
